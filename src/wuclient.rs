...
fn wu_client() {
    ...
    let client = Socket::new(Protocol::Sub0).unwrap();
    client.dial("tcp://0.0.0.0:5556").unwrap();


    let filter = "10001";
    let all_topics = filter.as_bytes().to_vec();
    client.set_opt::<Subscribe>(all_topics).unwrap();

    let mut total_temp = 0i64;
    let mut update_nbr = 0;
    while update_nbr < 100 {
        let msg = client.recv().unwrap();
        let msg = String::from_utf8(msg.as_slice().to_vec()).unwrap();
